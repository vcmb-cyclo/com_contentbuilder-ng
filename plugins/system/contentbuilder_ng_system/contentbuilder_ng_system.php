...
//        CBCompat::getPluginParams($this, 'system', 'contentbuilder_ng_system');
    ...
    function onAfterDispatch()
    {
            ...
            if (intval($pluginParams->get('is_auto_groups', 0)) == 1 && count($pluginParams->get('auto_groups', array()))) {
                ...
                $this->db->setQuery("
                        Select cv.userid, cv.verified_view, cv.verification_date_view, forms.verification_days_view, groups.group_id, groups.user_id
                            From 
                        (
                            #__contentbuilder_ng_users As cv,
                ...
                $this->db->setQuery("
                        ...
                        #__user_usergroup_map As groups
                            Left Join #__contentbuilder_ng_users As cv On ( cv.userid = groups.user_id And groups.group_id In (" . implode(',', $pluginParams->get('auto_groups', array())) . ") ) 
                        Where 
        ...
        if ($this->app->isClient('site')) {
            ...
            if ($the_ids) {
                ...
                $this->db->setQuery("Select Distinct forms.theme_plugin From #__contentbuilder_ng_forms As forms, #__contentbuilder_ng_articles As articles, #__content As content Where forms.id = articles.form_id And articles.article_id In (" . $the_ids . ") And content.id = articles.article_id And (content.state = 1 Or content.state = 0)");
            ...
            if ($pluginParams->def('disable_new_articles', 0) && trim(Factory::getApplication()->input->getCmd('option', '')) == 'com_content' && (trim(Factory::getApplication()->input->getCmd('task', '')) == 'new' || trim(Factory::getApplication()->input->getCmd('task', '')) == 'article.add' || (trim(Factory::getApplication()->input->getCmd('view', '')) == 'article' && trim(Factory::getApplication()->input->getCmd('layout', '')) == 'form') || (trim(Factory::getApplication()->input->getCmd('view', '')) == 'form' && trim(Factory::getApplication()->input->getCmd('layout', '')) == 'edit') && $a_id <= 0)) {
                $this->app->getLanguage()->load('com_contentbuilder_ng');
                $this->app->enqueueMessage(Text::_('COM_CONTENTBUILDER_NG_PERMISSIONS_NEW_NOT_ALLOWED'), 'error');
    ...
    function onAfterRoute()
    {
        ...
        if (Factory::getApplication()->input->getCmd('option', '') == 'com_content' || Factory::getApplication()->input->getCmd('option', '') == 'com_contentbuilder_ng') {
            // managing published states
            $date = Factory::getDate()->toSql();

            $this->db->setQuery("Update #__contentbuilder_ng_records Set published = 1 Where is_future = 1 And publish_up IS NOT NULL And publish_up <= '" . $date . "'");
            $this->db->execute();

            $this->db->setQuery("Update #__contentbuilder_ng_records Set published = 0 Where publish_down IS NOT NULL And publish_down <= '" . $date . "'");
            $this->db->execute();
        ...
        if ($option === 'com_contentbuilder_ng') {
            ...
            $this->db->setQuery("
                    Update 
                        #__contentbuilder_ng_records As records,
                        #__contentbuilder_ng_forms As forms,
                        #__contentbuilder_ng_registered_users As cbusers,
                        #__users As users
                    Set 
                        records.published = 0
                    Where
    ...
    function onAfterInitialize()
    {
        ...
        if ($app->isClient('site')) {
            ...
            $this->db->setQuery("
                    Update
                        #__contentbuilder_ng_articles As articles,
            ...
            $this->db->setQuery("
                ...
                From
                    #__contentbuilder_ng_records As records
                    Left Join #__contentbuilder_ng_forms As form On ( form.`type` = records.`type` And form.reference_id = records.reference_id )
                    Left Join #__contentbuilder_ng_articles As article On ( form.`type` = records.`type` And form.reference_id = records.reference_id And article.form_id = form.id And article.record_id = records.record_id )
            ...
            if (isset($list[0])) {
                ...
                $lang->load('com_contentbuilder_ng', JPATH_ADMINISTRATOR);
            }
            ...
            foreach ($list as $data) {
                ...
                if (is_array($data)) {
                    ...
                    if ($data['create_articles']) {
                        ...
                        if (count($ids)) {
                            $this->db->setQuery("Select Distinct `label`, reference_id From #__contentbuilder_ng_elements Where form_id = " . intval($data['form_id']) . " And reference_id In (" . implode(',', $ids) . ") And published = 1 Order By ordering");
                        ...
                        if ($article_id) {
                            $this->db->setQuery("Update #__contentbuilder_ng_articles Set `last_update`=" . $this->db->Quote($now) . " Where article_id = " . $this->db->Quote($article_id) . " And record_id = " . $this->db->Quote($data['record_id']) . " And form_id = " . $this->db->Quote($data['form_id']));
                            $this->db->execute();
                        }
                    }
                }
